    ...
    */
    socket.on('alert', (payload) => {
        if (payload.typeOfAlert === 'INSTANT_MSG') {
            const message = new message_1.default('!AYUDA¡', '¡¡¡Estoy en peligro!!!', 'El usuario ha enviado una alerta de emergencia.', typeOfRecord.ALERT);
            if (payload.roomId)
                socket.to(payload.roomId).emit('send-alert', message.newMessage);
        }
        if (payload.typeOfAlert === 'VOICE_MSG') {
            const message = new message_1.default('!AYUDA¡', payload.message, 'El usuario ha enviado una alerta de emergencia.', typeOfRecord.ALERT);
            if (payload.roomId)
                socket.to(payload.roomId).emit('send-alert', message.newMessage);
        }
    });
